...
class SQLGenerationRequest(BaseModel):
    ...
    database_schema: Optional[str] = Field(
        default=None, description="Database schema information (optional)"
    )
...
class NewChatRequest(BaseModel):
    ...
    session_name: Optional[str] = Field(
        default=None, description="Name for the new chat session"
    )
    initial_context: Optional[str] = Field(
        default=None, description="Initial context or instructions"
    )
