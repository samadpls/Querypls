...
def test_display_welcome_message(mock_st):
    with patch.object(st, "markdown") as mock_markdown:
        with patch.object(st, "session_state", MockSessionState()):
            initialize_session_state(messages=[{"role": "assistant", "content": "How may I help you?"}])
            display_welcome_message()
        mock_markdown.assert_called_once_with("#### Welcome to \n ## 🛢💬Querypls - Prompt to SQL")
...
def test_handle_new_chat(mock_db, mock_st):
    with patch("src.frontend.get_previous_chats") as mock_get_previous_chats:
        ...
        with patch.object(st, "markdown") as mock_markdown, patch.object(st, "button") as mock_button:
            with patch.object(st, "session_state", MockSessionState()):
...
def test_create_message():
    with patch.object(st, "session_state", MockSessionState()):
        ...
        create_message()
        assert st.session_state.messages == [{"role": "assistant", "content": "How may I help you?"}]
        assert st.session_state.key == "key"

def test_update_session_state(mock_db):
    chat = {"chat": [{"role": "user", "content": "Hello"}], "key": "new_key"}
    with patch.object(st, "session_state", MockSessionState()):
        initialize_session_state(messages=[{"role": "assistant", "content": "How may I help you?"}], key="old_key")
        with patch("src.frontend.database") as mock_database:
            update_session_state(mock_db, chat)
            mock_database.assert_called_once_with(mock_db, "old_key", [{"role": "assistant", "content": "How may I help you?"}])
