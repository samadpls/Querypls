...
def test_configure_page_styles(mock_open, mock_markdown, mock_set_page_config):
    ...
    mock_set_page_config.assert_called_once_with(page_title="Querypls", page_icon="💬", layout="wide")
...
def test_hide_main_menu_and_footer(mock_markdown):
    ...
    mock_markdown.assert_called_once_with("""<style>#MainMenu {visibility: hidden;}footer {visibility: hidden;}</style>""", unsafe_allow_html=True)
