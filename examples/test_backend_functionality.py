...
def test_sql_functionality():
    ...
    try:
        ...
        for query in sql_tests:
            ...
            try:
                # Test routing
                routing_decision = routing_service.determine_agent(
                    query, [], csv_loaded=False
                )
                print(f"Routing Decision: {routing_decision.agent}")

                # Test SQL generation
                request = SQLGenerationRequest(
                    user_query=query, conversation_history=[]
                )
    ...
    try:
        ...
        for query, expected_agent in test_cases:
            ...
            try:
                ...
                decision_with_csv = routing_service.determine_agent(
                    query, [], csv_loaded=True
                )
                print(f"Result (with CSV): {decision_with_csv.agent}")

                if (
                    decision.agent == expected_agent
                    or decision_with_csv.agent == expected_agent
                ):
                    print("✅ PASS")
                else:
                    print("❌ FAIL")
